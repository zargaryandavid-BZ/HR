...
/** Onboarding Automation page — manually created flows only */
export default function OnboardingAutomationPage() {
  ...
  const { data: automations, isLoading } = useQuery({
    queryKey: ["onboarding-automations"],
    queryFn: async () => {
  ...
  const positionsWithoutAutomation =
    positions?.filter((p) => p.isActive && p.onboardingTemplates.length === 0) ?? [];
  ...
  const createMutation = useMutation({
    ...
    onSuccess: async (positionId) => {
      await queryClient.invalidateQueries({ queryKey: ["onboarding-automations"] });
      await queryClient.invalidateQueries({ queryKey: ["positions-all"] });
      setCreateDialogOpen(false);
      setSelectedPositionId("");
      router.push(`/admin/settings/positions/${positionId}/onboarding-flow`);
    },
    onError: (error: Error) => {
  ...
  const duplicateMutation = useMutation({
    mutationFn: async ({
      ...
      const res = await fetch(
        `/api/settings/positions/${sourcePositionId}/onboarding-flow/duplicate`,
        {
          method: "POST",
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="Create position-based onboarding flows — each automation is tied to one position"
        actions={
          <Button
            onClick={() => {
              setSelectedPositionId("");
      ...
      <p className="text-sm text-muted-foreground">
        Each automation is position-based: one flow per position. Use{" "}
        <strong>Create Automation</strong> to pick a position and build its flow. Manage positions in{" "}
        <Link
