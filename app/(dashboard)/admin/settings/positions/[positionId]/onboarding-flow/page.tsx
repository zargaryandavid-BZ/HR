...
/** Onboarding flow builder — configure automation steps */
export default function OnboardingFlowPage({ params }: PageProps) {
  ...
  const saveStepMutation = useMutation({
    mutationFn: async (payload: {
      ...
      if (editingStep) {
        const res = await fetch(
          `/api/settings/positions/${positionId}/onboarding-flow/steps/${editingStep.id}`,
          {
            method: "PATCH",
        ...
        const res = await fetch(
          `/api/settings/positions/${positionId}/onboarding-flow/steps`,
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify(payload),
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["onboarding-flow", positionId] });
  ...
  const deleteStepMutation = useMutation({
    mutationFn: async (stepId: string) => {
      const res = await fetch(
        `/api/settings/positions/${positionId}/onboarding-flow/steps/${stepId}`,
        { method: "DELETE" }
      );
      const json = await res.json();
  ...
  async function copyPreviewLink() {
    const url = `${window.location.origin}/admin/settings/positions/${positionId}/onboarding-flow/preview`;
    try {
      await navigator.clipboard.writeText(url);
      setToast("Link copied — share with your team to review this flow.");
    } catch {
  ...
  return (
    <div className="space-y-8">
      <nav className="text-sm text-muted-foreground">
        <Link href="/admin/onboarding/automation" className="hover:underline">
          ← Onboarding
        </Link>
      </nav>
      ...
      <PageHeader
        ...
        description="Configure the onboarding automation steps new hires will complete"
        actions={
          <>
            <Button variant="outline" asChild>
              <a
                href={`/admin/settings/positions/${positionId}/onboarding-flow/preview`}
                target="_blank"
      ...
      {!hasAutomation && (
        <div className="rounded-md border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">
          ...
          <Link href="/admin/onboarding/automation" className="font-medium underline">
            Create one from Onboarding Automation
          </Link>{" "}
          before adding steps.
        </div>
      )}
      ...
      <div className="space-y-4">
        ...
        {orderedSteps.length === 0 && (
          <p className="text-sm text-muted-foreground text-center py-8">
            No steps yet. Add steps to build the onboarding flow.
          </p>
        )}
      </div>
