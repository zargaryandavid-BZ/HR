...
/** List active repository documents not yet assigned as offboarding docs */
export async function GET(request: NextRequest) {
  try {
    ...
    const assigned = await prisma.documentAssignment.findMany({
      where: { employeeId, isOffboarding: true },
      select: { sopId: true },
    });
    ...
    console.error("List available offboarding documents error:", error);
