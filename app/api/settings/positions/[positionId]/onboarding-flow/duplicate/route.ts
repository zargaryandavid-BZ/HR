...
/** Copy an onboarding automation and its steps to another position */
export async function POST(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const sourceTemplate = await prisma.onboardingTemplate.findFirst({
    ...
    const targetExisting = await prisma.onboardingTemplate.findFirst({
    ...
    const template = await prisma.$transaction(async (tx) => {
      const created = await tx.onboardingTemplate.create({
        data: {
          ...
          estimatedCompletionTime: sourceTemplate.estimatedCompletionTime,
