...
export async function PATCH(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const updated = await prisma.onboardingStep.findUnique({ where: { id: stepId } });
...
export async function DELETE(_request: NextRequest, { params }: RouteParams) {
  try {
    ...
    await prisma.$transaction(async (tx) => {
      await tx.onboardingStepProgress.deleteMany({ where: { stepId } });
      await tx.onboardingStep.delete({ where: { id: stepId } });

      const remaining = await tx.onboardingStep.findMany({
