...
export async function createEmployee(
  ...
  await sendWelcomeEmail(workEmail, fullName, tempPassword);

  if (input.phone) {
    const appUrl = getAppUrl();
    await sendSms(
      input.phone,
      `Welcome to Bazaar Printing HR! Log in at ${appUrl}/login with ${workEmail}. Check your email for your temporary password.`
    );
  }
