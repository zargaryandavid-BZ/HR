...
export function AssignDocumentModal({
  ...
  const { data: documents, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await fetch(`/api/documents/available?employeeId=${employeeId}`);
  ...
  const assignMutation = useMutation({
    mutationFn: async (documentIds: string[]) => {
      const res = await fetch(`/api/employees/${employeeId}/onboarding-docs`, {
        method: "POST",
    ...
    onSuccess: (result) => {
      ...
      queryClient.invalidateQueries({ queryKey: ["available-documents", employeeId] });
      onOpenChange(false);
      setSelected(new Set());
      setSearch("");
      onSuccess(
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="max-w-lg max-h-[85vh] flex flex-col">
        <DialogHeader>
          <DialogTitle>Assign Documents to {employeeName}</DialogTitle>
        </DialogHeader>
