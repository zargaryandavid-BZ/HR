...
/** Confirmation dialog before sending unsent offboarding docs to the employee portal */
export function SendOffboardingSignatureDialog({
  open,
  onOpenChange,
  employeeId,
  employeeName,
  onSuccess,
}: SendOffboardingSignatureDialogProps) {
  const queryClient = useQueryClient();

  const { data: unsentDocs, isLoading } = useQuery({
    queryKey: ["unsent-offboarding-docs", employeeId],
    queryFn: async () => {
      const res = await fetch(`/api/employees/${employeeId}/offboarding-docs/send`);
  ...
  const sendMutation = useMutation({
    ...
    onSuccess: (result) => {
      ...
      queryClient.invalidateQueries({ queryKey: ["unsent-offboarding-docs", employeeId] });
      onOpenChange(false);
      onSuccess(
  ...
  return (
    ...
    >
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle>Send offboarding documents for signature?</DialogTitle>
        </DialogHeader>
